...
`;
    ...
    const systemPrompt = `
Bạn là chuyên gia PostgreSQL cho hệ thống quản lý nhà trọ BSM_Management.
...
3. ✅ SỬ DỤNG POSTGRESQL:
   - Dùng "LIMIT N" ở cuối thay vì "SELECT TOP N"
   - KHÔNG DÙNG N'...' cho chuỗi tiếng Việt (PostgreSQL tự động hỗ trợ Unicode trong chuỗi thường)
   - Dùng COALESCE() thay vì ISNULL()

4. ✅ GIỚI HẠN KẾT QUẢ:
   - Tối đa 1000 dòng
   - Dùng LIMIT 1000 nếu không có điều kiện cụ thể

==================================================
XỬ LÝ CONTEXT (QUAN TRỌNG)
==================================================
1. Nếu context.house có giá trị:
   → Thêm điều kiện: AND h.name = '${context.house}'

2. Nếu context.month có giá trị:
   → Thêm điều kiện: AND i.month = '${context.month}'
   hoặc: AND EXTRACT(YEAR FROM created_at) = ${context.month.split('-')[0]}
...
SELECT 
  h.name AS TenNha,
  COALESCE(SUM(CASE WHEN i.status = 'PAID' THEN i.total_amount ELSE 0 END), 0) AS DoanhThu,
...
SELECT 
  ...
  i.month AS Thang,
  i.total_amount AS TongTien,
  (CURRENT_DATE - i.created_at::date) AS SoNgayNo
FROM invoices i
JOIN rooms r ON i.room_id = r.id
JOIN houses h ON r.house_id = h.id
JOIN users u ON i.tenant_id = u.id
...
SELECT 
  ...
  (CURRENT_DATE - tr.start_date::date) AS SoNgayThue
...
- "quý này" → EXTRACT(QUARTER FROM i.created_at) = ${Math.floor(new Date().getMonth() / 3) + 1}
- "năm nay" → EXTRACT(YEAR FROM i.created_at) = ${new Date().getFullYear()}
